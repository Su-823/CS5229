...
def random_crop_and_pad_image(image, crop_h, crop_w):
    """
    Randomly crop and pad the input image.
    Args:
      image: Training image to crop/pad.
      crop_h: Height of cropped segment.
      crop_w: Width of cropped segment.
    ...
    # 随机裁剪
    img_crop = tf.image.random_crop(image_pad, [crop_h, crop_w, tf.shape(image)[-1]])

    # 设置静态形状，确保形状在编译时已知
    img_crop.set_shape((crop_h, crop_w, image.shape[-1]))
...
def read_image_list(data_dir, data_list, count):
    f = open(data_list, 'r')
    images = []
...
def read_test_images_from_disk(input_queue, input_size, random_scale,
                          random_mirror=False):  # optional pre-processing arguments
    """Read one image and its corresponding mask with optional pre-processing.
...
class TestImageReader(object):
    ...
    def __init__(self, data_dir, data_list, data_id_list, input_size,
                 random_scale,
                 random_mirror, shuffle, coord, count=0):
        '''Initialise an ImageReader.
        ...
        self.image_list = read_image_list(
            self.data_dir, self.data_list, count)
        self.images = tf.convert_to_tensor(self.image_list, dtype=tf.string)
        self.queue = tf.compat.v1.train.slice_input_producer([self.images], shuffle=shuffle)
        self.image = read_test_images_from_disk(self.queue,
                                           self.input_size,
                                           random_scale,
                                           random_mirror)
